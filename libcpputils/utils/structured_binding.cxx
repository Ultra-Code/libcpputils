...
struct binding
{
    std::uint64_t integer{}; //!< A 64bit unisigned int.
    std::string string{};    //!< A std::string.
};

auto arrayreturn() -> auto
{
    auto const a{ std::experimental::make_array(1, 2, 3) };
    return a;
}

auto maps() -> decltype(auto)
{
    return std::unordered_map<std::uint32_t, std::string>(
      { { 1, "one" }, { 2, "two" } });
}

auto use_structures(int argc, char *argv[]) -> int
{
 ...
 */
auto whatsHappenningInTheStructure(int v /*!< [in,out]  @param v no input.*/) -> void
{
    auto structure /*!< structures */ { binding{ .integer = 1,
                                                 .string = { "Nothing" } } };
    ...
    std::cout << "Var contains " << var << " which is the value of" << s
              << " oopus now moved" << structure.string << " .string Now big ooopus\n";
