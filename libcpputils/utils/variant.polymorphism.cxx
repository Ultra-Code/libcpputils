...
class Coord
{
   ...
   public:
    ...
    constexpr auto operator+=(const Coord& rhs) -> std::uint16_t
    {
        return this->lenght += rhs.lenght;
    }
    constexpr auto operator+(const Coord& rhs) noexcept -> std::uint16_t
    {
        return *this += rhs;
    }
};
   ...
   public:
    ...
    auto draw() const
    {
        std::cout << "Line of " << static_cast<std::uint16_t>(m_left_side)
                  << "m at left_side and " << static_cast<std::uint16_t>(m_right_side)
                  << "m at right_side" << '\n';
    }
};
using poly_variant = std::variant<Line, Circle>;
auto make_plane_figure() -> std::vector<poly_variant>
{
    ...
    return std::move(figure_part);
}
auto polymorphic_variant() noexcept -> void
{
    try
    {
        const auto&& figure = make_plane_figure();
        for (const auto& parts : figure)
        {
            std::cout << "index : " << parts.index() << '\n';
            std::visit([](const auto& variant) { variant.draw(); }, parts);
        }
    }
